...
export const ImplementationResultSchema = z.object({
    ...
    result => result.status === 'success' || (!!result.currentRequirementId && !!result.conflictingRequirementId && !!result.explanation),
    { message: 'currentRequirementId, conflictingRequirementId and explanation are all required when status is "conflict"' },
);
