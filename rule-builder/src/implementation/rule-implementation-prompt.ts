...
- Every entry in the control's \`FINDINGS\` record MUST contain an \`issue\` and a \`remediation\`.
- \`issue\` MUST describe the problem detected by that specific finding, NEVER remediation guidance.
- \`issue\` MUST be IaC-format-agnostic: no CloudFormation property names, Terraform argument names, resource type ARNs, or format-specific terminology.
- \`remediation\` MUST provide clear, IaC-format-agnostic instructions for resolving the corresponding issue.
- \`evaluate()\` MUST return a typed \`FindingKey\` derived from \`keyof typeof FINDINGS\`, or \`null\` when the resource complies.
