import { describe, it, expect } from 'vitest';
import { ESH007Rule } from '../../../../../../src/assess/scanning/security-matrix/rules/opensearch/007-zone-awareness.cf.js';
import { CloudFormationResource } from '../../../../../../src/assess/scanning/security-matrix/security-rule-base.js';

describe('ESH007Rule', () => {
  const rule = new ESH007Rule();
  const stackName = 'test-stack';

  function createOpenSearchResource(props: Record<string, any> = {}): CloudFormationResource {
    return {
      Type: 'AWS::OpenSearchService::Domain',
      Properties: {
        DomainName: 'test-domain',
        ...props
      },
      LogicalId: 'TestOpenSearchDomain'
    };
  }

  it('should have correct rule properties', () => {
    expect(rule.id).toBe('ESH-007');
    expect(rule.priority).toBe('HIGH');
    expect(rule.appliesTo('AWS::OpenSearchService::Domain')).toBe(true);
    expect(rule.appliesTo('AWS::Elasticsearch::Domain')).toBe(false);
  });

  it('should detect missing ClusterConfig', () => {
    const domain = createOpenSearchResource();
    const result = rule.evaluate(domain, stackName);
    
    expect(result).not.toBeNull();
    expect(result?.issue).toContain('OpenSearch domain not configured for zone awareness');
  });

  it('should detect ZoneAwarenessEnabled set to false', () => {
    const domain = createOpenSearchResource({
      ClusterConfig: {
        ZoneAwarenessEnabled: false
      }
    });
    const result = rule.evaluate(domain, stackName);
    
    expect(result).not.toBeNull();
  });

  it('should pass with ZoneAwarenessEnabled set to true', () => {
    const domain = createOpenSearchResource({
      ClusterConfig: {
        ZoneAwarenessEnabled: true
      }
    });
    const result = rule.evaluate(domain, stackName);
    
    expect(result).toBeNull();
  });


});