...
export class TfNeptune005Rule extends BaseTerraformRule {
  constructor() {
    super(
      'NEPTUNE-005',
      'HIGH',
    ...
    for (const sgId of vpcSecurityGroupIds) {
      ...
      if (sg && this.hasUnrestrictedIngress(sg, allResources)) {
        return this.createScanResult(
          ...
          `Remove ingress rules allowing 0.0.0.0/0 from security group ${sg.address}.`
        );
      }
    }
...
export default new TfNeptune005Rule();
