/**
 * DEPRECATED - LAMBDA-002 is disabled and unregistered.
 *
 * The check could never confirm a violation. It matched environment variable
 * names against keyword patterns, then decided the value was safe if the value
 * contained a keyword like "secretsmanager" or "kms", or referenced a logical ID
 * whose name contained "key" or "parameter". That produced noise on names like
 * PARTITION_KEY and AUTH_URL while passing real plaintext secrets, so a HIGH
 * severity violation was not defensible.
 *
 * MIGRATION: reinstate this as a warning once the warning system exists, for
 * findings that may be an issue but cannot be proven one. The name and value
 * heuristics below are the starting point; the Ref/GetAtt name-guessing and the
 * FindInMap/If/Select keyword pass should not carry over as-is.
 *
 * Unit tests for the original implementation are in git history at commit a4b496e
 * (tests/core/scanners/srt/rules/lambda/002-secrets-manager.test.ts).
 */
